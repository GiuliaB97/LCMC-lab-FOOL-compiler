...
instruction:
        PUSH n=INTEGER{					/*n mi da la possibilità di accedere al lessema che ha matchato con integer*/
        			...
        			code[i++] =	Integer.parseInt($n.text);}//ho bisogno di due caselle perchè mi serve l'argomento
	  | PUSH l=LABEL 	 {	code[i++] = PUSH;			/*push the location address pointed by LABEL on the stack;*/
	  						labelRef.put(i++, $l.text);}// mettere sullo stack l'indirizzo a cui è associato label	     
	   ...
	   * Aspetto di arrivare alla fine per riempire con un cicclo for il labelRef con i labelDef.
	  ...
	  */
	  ...
	  | STORETM     {code[i++] = STORETM;}//pop the top of the stack and copy it in the TM register, ha argomenti implici perchè lavora con lo stack
